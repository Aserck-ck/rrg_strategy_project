...
from rrg_stock_a.models.module import SequentialNonOverlappingSampler
...
def prepare_sw1_dataloaders_pro(lookback_window, predict_window, batch_size,
    ...
    val_list = [x.iloc[-bound_train:-bound_val] for x in data_list]
    test_list = [x.iloc[-bound_val:] for x in data_list]
...
def prepare_sw1_predict_dataset(lookback_window, z_score_feature, log_feature, origin_feature, stamp_feature):
    ...
    test_list = [x.iloc[-bound_val-5:] for x in data_list]
...
def prepare_sw2_testdata(time_window, predict_window):
    ratio_df = get_sw_level2_ratio(ts_code='all', frequency='daily').set_index('trade_date')
    momentum_df = get_sw_level2_momentum(ts_code='all', frequency='daily').set_index('trade_date')
    ratio_df.drop(columns=['000300.SH'], inplace=True)
    momentum_df.drop(columns=['000300.SH'], inplace=True)

    abs_ratio_df = get_sw_level2_ratio(ts_code='all', frequency='daily', get_abs=True).set_index('trade_date')
    abs_momentum_df = get_sw_level2_momentum(ts_code='all', frequency='daily', get_abs=True).set_index('trade_date')
    abs_ratio_df.drop(columns=['000300.SH'], inplace=True)
    abs_momentum_df.drop(columns=['000300.SH'], inplace=True)

    ratio_train_df = ratio_df.iloc[-2000:-500]
    ratio_test_df = ratio_df.iloc[-500:]
    abs_ratio_train_df = abs_ratio_df.iloc[-2000:-500]
    abs_ratio_test_df = abs_ratio_df.iloc[-500:]

    ratio_train = np.array(ratio_train_df)
    ratio_test = np.array(ratio_test_df)
    abs_ratio_train = np.array(abs_ratio_train_df)
    abs_ratio_test = np.array(abs_ratio_test_df)

    momentum_train_df = momentum_df.iloc[-2000:-500]
    momentum_test_df = momentum_df.iloc[-500:]
    abs_momentum_train_df = abs_momentum_df.iloc[-2000:-500]
    abs_momentum_test_df = abs_momentum_df.iloc[-500:]

    momentum_train = np.array(momentum_train_df)
    momentum_test = np.array(momentum_test_df)
    abs_momentum_train = np.array(abs_momentum_train_df)
    abs_momentum_test = np.array(abs_momentum_test_df)

    scaler_ratio = StandardScaler()
    scaler_momentum = StandardScaler()

    # Merge all sectors in one and scale
    ratio_train_scaled = scaler_ratio.fit_transform(ratio_train.reshape(-1, 1))
    ratio_test_scaled = scaler_ratio.transform(ratio_test.reshape(-1, 1))
    abs_ratio_train_scaled = scaler_ratio.transform(abs_ratio_train.reshape(-1, 1))
    abs_ratio_test_scaled = scaler_ratio.transform(abs_ratio_test.reshape(-1, 1))

    momentum_train_scaled = scaler_momentum.fit_transform(momentum_train.reshape(-1, 1))
    momentum_test_scaled = scaler_momentum.transform(momentum_test.reshape(-1, 1))
    abs_momentum_train_scaled = scaler_momentum.transform(abs_momentum_train.reshape(-1, 1))
    abs_momentum_test_scaled = scaler_momentum.transform(abs_momentum_test.reshape(-1, 1))

    ratio_train_scaled = ratio_train_scaled.reshape(ratio_train.shape[0], ratio_train.shape[1])
    ratio_test_scaled = ratio_test_scaled.reshape(ratio_test.shape[0], ratio_test.shape[1])
    abs_ratio_train_scaled = abs_ratio_train_scaled.reshape(abs_ratio_train.shape[0], abs_ratio_train.shape[1])
    abs_ratio_test_scaled = abs_ratio_test_scaled.reshape(abs_ratio_test.shape[0], abs_ratio_test.shape[1])

    momentum_train_scaled = momentum_train_scaled.reshape(momentum_train.shape[0], momentum_train.shape[1])
    momentum_test_scaled = momentum_test_scaled.reshape(momentum_test.shape[0], momentum_test.shape[1])
    abs_momentum_train_scaled = abs_momentum_train_scaled.reshape(abs_momentum_train.shape[0],
                                                                  abs_momentum_train.shape[1])
    abs_momentum_test_scaled = abs_momentum_test_scaled.reshape(abs_momentum_test.shape[0], abs_momentum_test.shape[1])

    ratio_train_scaled = pd.DataFrame(index=ratio_train_df.index,
                                      columns=ratio_train_df.columns, data=ratio_train_scaled)
    ratio_test_scaled = pd.DataFrame(index=ratio_test_df.index,
                                     columns=ratio_test_df.columns, data=ratio_test_scaled)
    abs_ratio_train_scaled = pd.DataFrame(index=abs_ratio_train_df.index,
                                          columns=abs_ratio_train_df.columns, data=abs_ratio_train_scaled)
    abs_ratio_test_scaled = pd.DataFrame(index=abs_ratio_test_df.index,
                                         columns=abs_ratio_test_df.columns, data=abs_ratio_test_scaled)

    momentum_train_scaled = pd.DataFrame(index=momentum_train_df.index,
                                         columns=momentum_train_df.columns, data=momentum_train_scaled)
    momentum_test_scaled = pd.DataFrame(index=momentum_test_df.index,
                                        columns=momentum_test_df.columns, data=momentum_test_scaled)
    abs_momentum_train_scaled = pd.DataFrame(index=abs_momentum_train_df.index,
                                             columns=abs_momentum_train_df.columns, data=abs_momentum_train_scaled)
    abs_momentum_test_scaled = pd.DataFrame(index=abs_momentum_test_df.index,
                                            columns=abs_momentum_test_df.columns, data=abs_momentum_test_scaled)

    train_dataset = [ratio_train_scaled, abs_ratio_train_scaled,
                     momentum_train_scaled, abs_momentum_train_scaled]

    test_dataset = [ratio_test_scaled, abs_ratio_test_scaled,
                    momentum_test_scaled, abs_momentum_test_scaled]
    test_data={}

    for col in ratio_test_scaled.columns:
        symbol_data=[]
        for dataset in test_dataset:
            symbol_data.append(dataset[col].values)
        symbol_data = np.array(symbol_data).T
        test_data[col] = torch.from_numpy(symbol_data)

    return test_dataset, test_data, scaler_ratio, scaler_momentum


if __name__ == "__main__":
    from train_sw2 import get_config
    config = get_config()
    train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset, train_sampler=(
        prepare_sw2_dataloaders(config.time_window, config.predict_window, config.batch_size,
