...
def plot_result(benchmark):
    ...
    for col in [benchmark]:
        ...
        plt.title(f"{col} - 周度调仓策略 (交易成本: 千3)")
            ...
            try:
                ...
                for ratio_df, momentum_df in zip(self.ratio_df_list, self.momentum_df_list):
                    ...
                    condition1l = (self.momentum_weekly_df.loc[prev_date]-self.momentum_weekly_df.loc[prev_prev_date] >
                                self.momentum_weekly_df.loc[prev_prev_date]-self.momentum_weekly_df.loc[prev_prev_date2])
                    ...
                    condition4 = momentum_df.loc[current_date] > momentum_df.loc[prev_date] + 0.5
                    condition5 = momentum_df.loc[current_date] > ratio_df.loc[current_date] -1
                    condition6 = ratio_df.loc[current_date] < 100
                    ...
                    condition7 = (ratio_df.loc[current_date] > ratio_df.loc[current_date].median()) & (momentum_df.loc[current_date] > momentum_df.loc[current_date].median())
                    condition8 = momentum_df.loc[current_date] - momentum_df.loc[prev_date] > max((momentum_df.loc[current_date] - momentum_df.loc[prev_date]).median(),1)
                    condition9 = ratio_df.loc[current_date] - ratio_df.loc[prev_date] > max((ratio_df.loc[current_date] - ratio_df.loc[prev_date]).median(),1)

                    # condition7l = (self.momentum_weekly_df.loc[prev_date] > self.momentum_weekly_df.loc[prev_date].median()) & (self.ratio_weekly_df.loc[prev_date] > self.ratio_weekly_df.loc[prev_date].median())
                    # condition8l = self.momentum_weekly_df.loc[prev_date] - self.momentum_weekly_df.loc[prev_prev_date] > max((self.momentum_weekly_df.loc[prev_date] - self.momentum_weekly_df.loc[prev_prev_date]).median(),1)
                    # condition9l = self.ratio_weekly_df.loc[prev_date] - self.ratio_weekly_df.loc[prev_prev_date] > max((self.ratio_weekly_df.loc[prev_date] - self.ratio_weekly_df.loc[prev_prev_date]).median(),1)
                    ...
                    momentum_diff_l = self.momentum_weekly_df.loc[prev_date] - self.momentum_weekly_df.loc[
                        prev_prev_date]
...
steps=2
x_pred_df = pd.read_csv(f'./analyse/sw1_x_pred_mean2_weekly.csv',index_col=0,parse_dates=True)
y_pred_df = pd.read_csv(f'./analyse/sw1_y_pred_mean2_weekly.csv',index_col=0,parse_dates=True)
...
x_pred_df5 = pd.read_csv(f'./analyse/rolling/stack_rg_x_pred3_walk_forward_360.csv',index_col=0,parse_dates=True)
y_pred_df5 = pd.read_csv(f'./analyse/rolling/stack_rg_y_pred3_walk_forward_360.csv',index_col=0,parse_dates=True)
...
debt = pd.read_csv('000012.SH.csv',index_col=0,parse_dates=True)

close_df['000300.SH'] = hs300['close']
close_df['000012.SH'] = debt['close']

close_df=close_df.dropna()
amt_df['000300.SH'] = hs300['amount']
amt_df=amt_df.dropna()
start_date = x_pred_df.index[0]
end_date = x_pred_df.index[-300]
...
pct_change_df = spx_weekly_df.pct_change().dropna()
start_date1 = x_pred_df.index[-845]
...
x_pred = [ratio_w_df]
y_pred = [momentum_w_df]


test = BackTest(start_date1, end_date, x_pred,y_pred
                , ratio_w_df, momentum_w_df, pct_change_df,'000012.SH')
