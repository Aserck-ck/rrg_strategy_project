...
def test_model():
    ...
    sizes = [128, 256,512]
    model_name = [f'sw_level1_w_stack{predict_window}_{size}' for size in sizes]
    ...
    for i in range(dataset_len - 1, -1, -10):
        ...
        for j in range(31):
            ...
            point_x = (raw_df['ratio'].iloc[-predict_window - 5:-5, j] + 100) * (
                        preds[0, :, j, 0] * std_df['ratio_chg5'].iloc[j] + 1) - 100
            point_x = np.insert(preds[0, :, j, 0] * std_df['ratio'].iloc[j], 0,
                                raw_df['ratio'].iloc[-predict_window - 1, j])

            point_y = (raw_df['momentum'].iloc[-predict_window - 5:-5, j] + 100) * (
                        preds[0, :, j, 1] * std_df['momentum_chg5'].iloc[j] + 1) - 100
            point_y = np.insert(preds[0, :, j, 1] * std_df['momentum'].iloc[j], 0,
                                raw_df['momentum'].iloc[-predict_window - 1, j])
...
def test_trail2(predict_step=0,dataset='sw1'):
    sys.path.append('/')
    print(torch.cuda.is_available())
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"使用设备: {device}")
    ...
    test_dataset = (
        prepare_sw1_backtest_weekly(time_window, predict_window, z_score_feature,
                                    log_feature, origin_feature, stamp_feature,dataset=dataset))
    if dataset=='sw1':
        etf_info_df = get_sw_level1_info()
        num_industries = 31

    elif dataset=='sw2':
        from market_data.sw_level2.sw_level2_functions import get_sw_level2_info
        etf_info_df = get_sw_level2_info()
        num_industries = 124
    else:
    ...
    x_pred_df = pd.DataFrame(index=date_index, columns=raw_df['ratio'].columns,
                             data=np.zeros((date_index.shape[0], num_industries)))
    y_pred_df = pd.DataFrame(index=date_index, columns=raw_df['ratio'].columns,
                             data=np.zeros((date_index.shape[0], num_industries)))
    ...
    with torch.no_grad():
        for i in range(dataset_len):
            ...
            test_data, stamps = test_dataset[i]
            test_data = test_data.float()
            stamps = stamps.unsqueeze(0).to(device)
            test_data = test_data.unsqueeze(0)
            test_data = test_data.to(device)
            stamps_in = stamps[:, :time_window].clone()
            test_in = test_data[:, :time_window].clone()
            preds = []

            for model_x in model:
                preds.append(model_x(test_in,stamps_in,top_k=5))
            preds = torch.stack(preds, dim=-1)
            preds = preds.mean(dim=-1)
            # preds = torch.cat([test_in[:,-1:,:,:2],preds],dim=1)
            preds = preds.cpu().detach().numpy()
    ...
    with open(f"./analyse/{dataset}_x_predict_mean{predict_window}_weekly.pkl", 'wb') as f:
        pickle.dump(x_pkl, f)
    with open(f"./analyse/{dataset}_y_predict_mean{predict_window}_weekly.pkl", 'wb') as f:
        pickle.dump(y_pkl, f)
...
def calc_trail(window=3,dataset='sw1'):
    with open(f"./analyse/{dataset}_x_predict_mean{window}_weekly.pkl", 'rb') as f:
        x_data = pickle.load(f)

    with open(f"./analyse/{dataset}_y_predict_mean{window}_weekly.pkl", 'rb') as f:
    ...
    if dataset=='sw1':
        ...
        ratio_w_df = get_sw_level2_ratio(ts_code='all', frequency='weekly').set_index('trade_date').drop(
            columns=['000300.SH'])
        moment_w_df = get_sw_level2_momentum(ts_code='all', frequency='weekly').set_index('trade_date').drop(
            columns=['000300.SH'])
    ...
    x_pred_df.to_csv(f'./analyse/{dataset}_x_pred_mean{window}_weekly.csv')
    y_pred_df.to_csv(f'./analyse/{dataset}_y_pred_mean{window}_weekly.csv')

    return


def test_loss(window_idx=0,size_idx=0,dataset='sw1'):
    sys.path.append('/')
    print(torch.cuda.is_available())
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"使用设备: {device}")
    ...
    config = get_config()
    time_window = config.time_window
    predict_window = config.predict_window[window_idx]
    batch_size = config.batch_size
    z_score_feature = config.z_score_feature
    log_feature = config.log_feature
    origin_feature = config.origin_feature
    ...
    size =config.hidden_size[size_idx]
    model_name = f'sw_level1_w_stack{predict_window}_{size}'
    ...
    model = Stacked_GRU_GAT_Predictor(
        ...
        hidden_size=size, # 使用当前测试的 size
        gat_heads=config.d_out*2,
        num_gru_layers=config.num_gru_layers,
        num_gat_layers=config.num_gat_layers,
        pred_len=predict_window, # 使用当前测试的 predict_window
        dropout=config.dropout, # 确保你的config里有k_gat
    ).to(device)

    # 2. 加载状态字典
    # 假设 model_path 文件中保存的是 state_dict
    ...
    train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset, train_sampler = (
        prepare_sw1_dataloaders_weekly(time_window, predict_window, batch_size,
                                       z_score_feature, log_feature, origin_feature, stamp_feature,
                                       dataset=dataset,train_ratio=0.2,val_ratio=0.1,test_ratio=0.7))
    dataset_len = len(test_dataset)
...
if __name__ == "__main__":
    ...
    calc_trail(2,'sw1')
    # for step in [0,1]:
    #     test_trail2(step,'sw2')
    #     calc_trail(step + 2,'sw2')
    # plot_trail()
