...
if __name__ == '__main__':
    ...
    ratio_model = torch.load(os.path.join(file_path, 'lstm_models', 'sw_level1_ratio.pth'), weights_only=False)
    momentum_model = torch.load(os.path.join(file_path, 'lstm_models', 'sw_level1_momentum.pth'), weights_only=False)
    ...
    # Get crowdedness
    crowdedness_df = get_sw_level1_crowdedness(ts_code='all', frequency='daily').set_index('trade_date')
    ...
    # Load scalers
    scaler_ratio = load(os.path.join(file_path, 'lstm_models', 'sw_level1_ratio_scaler.joblib'))
    scaler_momentum = load(os.path.join(file_path, 'lstm_models', 'sw_level1_momentum_scaler.joblib'))
    ...
    for etf_code in momentum_df.columns[:]:
        ...
        for i, (x, y) in enumerate(zip(ratio_all_df[etf_code].iloc[-tail_length:-1], momentum_all_df[etf_code].iloc[-tail_length:-1])):
            plt.arrow(x, y, \
                    ...
                    head_width = 0.1, linewidth=0, fill=True, color='black')
    ...
    plt.title('Date: '+ratio_all_df.index[-2].strftime('%Y-%m-%d')+'; Frequency: daily', fontsize=30)
    plt.axis('off')

    plt.savefig(os.path.join(file_path, 'graphs', 'rrg_daily', \
