...
def create_pooled_dataset(config):
    ...
    for code in tqdm(sector_codes, desc="处理板块数据"):
        ...
        # 标准化整个序列
        ratio_mean, ratio_std = ratio_data.mean(), ratio_data.std()
        momentum_mean, momentum_std = momentum_data.mean(), momentum_data.std()
        ratio_norm = (ratio_data - ratio_mean) / (ratio_std + 1e-8)
        momentum_norm = (momentum_data - momentum_mean) / (momentum_std + 1e-8)

        for i in range(len(momentum_data) - config.time_window - config.predict_window + 1):
            feature = np.column_stack((
                ratio_norm[i:(i + config.time_window)],
                momentum_norm[i:(i + config.time_window)]
            ))
            all_X.append(feature)
            
            target_ratio = ratio_norm[i + config.time_window + config.predict_window - 1]
            target_momentum = momentum_norm[i + config.time_window + config.predict_window - 1]
            all_Y.append([target_ratio, target_momentum])
...
def train_single_general_model(config):
    ...
    if config.model_type == 'LSTM':
        model = LSTMModel(config.input_size, config.output_size, config.hidden_size, config.dropout).to(config.device)
    else:
        model = GRUModel(config.input_size, config.output_size, config.hidden_size, config.dropout).to(config.device)
...
def run_prediction_with_general_model(config):
    """使用单一通用模型为所有板块生成预测"""
    print(f"\n====== 阶段二: 使用单一通用 {config.model_type} 模型生成预测文件 ======")
    ...
    with torch.no_grad():
        for code in tqdm(sector_codes, desc="为所有板块生成预测"):
            if code not in all_ratio_df.columns: continue

            ratio_series = all_ratio_df[code]
            momentum_series = all_momentum_df[code]
            ratio_mean, ratio_std = ratio_series.mean(), ratio_series.std()
            momentum_mean, momentum_std = momentum_series.mean(), momentum_series.std()
            ratio_norm = (ratio_series - ratio_mean) / (ratio_std + 1e-8)
            momentum_norm = (momentum_series - momentum_mean) / (momentum_std + 1e-8)

            for i in range(len(momentum_series) - config.time_window - config.predict_window + 1):
                predict_date = momentum_series.index[i + config.time_window + config.predict_window - 1]
                
                feature = np.column_stack((
                    ratio_norm.iloc[i:(i + config.time_window)],
                    momentum_norm.iloc[i:(i + config.time_window)]
                ))
                input_tensor = torch.FloatTensor(feature).unsqueeze(0).to(config.device)
                
                prediction = model(input_tensor).squeeze().cpu().numpy()
                
                pred_ratios_df.loc[predict_date, code] = prediction[0]* ratio_std + ratio_mean
                pred_momentums_df.loc[predict_date, code] = prediction[1]* momentum_std + momentum_mean

    # 3. 保存结果
    pred_ratios_df.dropna(how='all', inplace=True)
    pred_momentums_df.dropna(how='all', inplace=True)
    
    x_pred_filename = os.path.join(config.results_save_dir, f'x_pred_single_{config.model_type.lower()}_weekly.csv')
    y_pred_filename = os.path.join(config.results_save_dir, f'y_pred_single_{config.model_type.lower()}_weekly.csv')
